...
public class SipSampler extends AbstractSampler implements TestStateListener, TestIterationListener {
    ...
    private void readSessionData() {
        ...
        if (identification == null || "".equals(identification)) {
            theSessionData = sipNodeContext
                    .readSessionData("SipSessionData." + sipNodeName + "." + dialogueNb + "." + threadnb);
        } else {
            theSessionData = sipNodeContext
                    .readSessionData("SipSessionData." + sipNodeName + "." + identification + "." + dialogueNb);
        }
    ...
    private void resetSessionData() {
        ...
        if (theSessionData != null) {
            ...
            if (identification == null || "".equals(identification)) {
                ...
                sipNodeContext
                        .resetSessionData("SipSessionData." + sipNodeName + "." + identification + "." + dialogueNb);
            }
        }
    ...
    private void storeSessionData() {
        ...
        if (identification == null || "".equals(identification)) {
            sipNodeContext.storeSessionData("SipSessionData." + sipNodeName + "." + dialogueNb + "." + threadnb,
                    theSessionData);
        } else {
            sipNodeContext.storeSessionData("SipSessionData." + sipNodeName + "." + identification + "." + dialogueNb,
                    theSessionData);
        }
    }
    ...
    private void readSessionDataFirst() {
        ...
        int threadnb = getThreadContext().getThreadNum();
        if (identification == null || "".equals(identification)) {
            theSessionData = (SipDialogData) variables
                    .getObject("SipSessionData." + sipNodeName + "." + dialogueNb + "." + threadnb);
        } else {
            theSessionData = (SipDialogData) variables
                    .getObject("SipSessionData." + sipNodeName + "." + identification + "." + dialogueNb);
        }
        if (theSessionData == null) {
            theSessionData = new SipDialogData(sipNodeContext);
        }
    ...
    private void resetSessionDataFirst() {
        ...
        if (theSessionData != null) {
            ...
            theSessionData = null;
            if (identification == null || "".equals(identification)) {
                variables.remove("SipSessionData." + sipNodeName + "." + getPropertyAsString("sip.message.dialnb") + "."
                        + threadnb);
            } else {
                variables.remove("SipSessionData." + sipNodeName + "." + identification + "."
                        + getPropertyAsString("sip.message.dialnb"));
            }
        }
        if (sipNodeContext == null) {
            sipNodeContext = getSipNodeContext(getPropertyAsString("sip.node.name"));
    ...
    private void resetHeaderMap() {
        ...
        int i = 0;
        while (getPropertyAsString("sip.header.name." + i) != null
                && !"".equals(getPropertyAsString("sip.header.name." + i))) {
            headersMap.addHeader(getPropertyAsString("sip.header.name." + i),
                    getPropertyAsString("sip.header.value." + i));
            i++;
        }
    }
        ...
        for (java.util.Map.Entry<String, List<String>> entry : headersMap.entrySet()) {
            ...
            for (String oneVal : entry.getValue()) {
                ...
                if (!MISSING.equals(oneVal)) {
                    Boolean added = false;
                    try {
                        SIPHeaderList headers = (SIPHeaderList) headerFactory
                                .createHeaders(entry.getKey() + ":" + oneVal);
                                               request.addHeader(headers);
                        added = true;
                    } catch (Exception e) {
                        _logger.info("add header failed. will try without expecting a list. {} : {}", entry.getKey(),
                                entry.getValue());
                    }
                    if (!added) {
    ...
    public SampleResult sample(Entry arg0) {
        ...
        if (sipNodeContext == null) {
            if (_logger.isDebugEnabled())
                _logger.debug("sipNodeContext is null");
        ...
        if (!sipNodeContext.isNodeReady()) {
            if (_logger.isDebugEnabled())
                _logger.debug("sipNode not ready : {}", sipNodeContext.getNotReadyReason());
        ...
        if (_logger.isDebugEnabled())
            _logger.debug("sampler sample {} start for node name {} : {} : {} : {} :: {} : {}",
                    new Object[] { getName(), sipNodeName, direction, isRequest, dialogueNb, transactionNb });
        ...
        if (_logger.isDebugEnabled())
            _logger.debug("sipNodeContext.getSipNode().getTraficType() {}", trafficType);
        ...
        if (_logger.isDebugEnabled())
            _logger.debug("identification {} : {}", identification,
                    sipNodeContext.getSipNode().getIdentificationHeader());
        ...
        if (_logger.isDebugEnabled())
            _logger.debug("sample: {} : {} :: {} : {}: {}",
                    new Object[] { direction, isRequest, dialogueNb, transactionNb, relatedTransactionNb });
        ...
        if ("send".equals(direction)) {
            ...
            if (_logger.isDebugEnabled())
                _logger.debug("sample unknown {}", direction);
        }
        if (_logger.isDebugEnabled()) {
            long diff = dateToString() - start;
            if (diff > 10 && _logger.isWarnEnabled())
                _logger.warn("sampler sample for node name  {} : {} : {} :: {} {} -> RUN took {}",
                        new Object[] { sipNodeName, direction, isRequest, dialogueNb, transactionNb, diff });
        }
        res.sampleEnd();
        // if (reset) {
        // resetSessionData();
    ...
    private void sendMessage(SampleResult res) {
        ...
        if (isRequest) {
            ...
            if (res.isSuccessful()) {
                SimonManager.getCounter(SipCounters.SIP_NODE_OUTBOUND_ANSWER.toString()
                        + SipCounters.SUCCESS_SUFFIX.toString() + "." + getPropertyAsString("sip.message.response.code")
                        + "." + sipNodeContext.getSipNode().getSipNodeName().replace(".", "_")).increase();
            } else {
                SimonManager.getCounter(SipCounters.SIP_NODE_OUTBOUND_ANSWER.toString()
                        + SipCounters.ERROR_SUFFIX.toString() + "." + getPropertyAsString("sip.message.response.code")
                        + "." + sipNodeContext.getSipNode().getSipNodeName().replace(".", "_")).increase();
            }
    ...
    private void sendRequestMessage(SampleResult res, String command) {
        ...
        if (res.isSuccessful()) {
            SimonManager
                    .getCounter(SipCounters.SIP_NODE_OUTBOUND_REQUEST.toString() + SipCounters.SUCCESS_SUFFIX.toString()
                            + "." + command + "." + sipNodeContext.getSipNode().getSipNodeName().replace(".", "_"))
                    .increase();
        } else {
            SimonManager
                    .getCounter(SipCounters.SIP_NODE_OUTBOUND_REQUEST.toString() + SipCounters.ERROR_SUFFIX.toString()
                            + "." + command + "." + sipNodeContext.getSipNode().getSipNodeName().replace(".", "_"))
                    .increase();
        }
    }
    ...
    private void sendRequestNewDialog(SampleResult res, String command) {
        if (_logger.isDebugEnabled())
            _logger.debug("sendRequestNewDialog {}", command);
        String result = "";
        ...
        try {
            ...
            if (reqUri == null || "".equals(reqUri)) {
                if (_logger.isDebugEnabled())
                    _logger.debug("Request-URI header not set.");
                ...
                if (requestURI == null) {
                    if (_logger.isDebugEnabled())
                        _logger.debug("Request-URI badly formated.");
            ...
            if (cseqString != null) {
                ...
                if (cseqparts.length == 2) {
                    cSeqHeader = headerFactory.createCSeqHeader(Long.parseLong(cseqparts[0]), cseqparts[1]);
                } else if (cseqparts.length == 1) {
                    cSeqHeader = headerFactory.createCSeqHeader(Long.parseLong(cseqparts[0]), command);
                } else {
            ...
            if (maxF != null) {
                maxForwards = headerFactory.createMaxForwardsHeader(Integer.parseInt(maxF));
            } else {
                if (_logger.isDebugEnabled())
                    _logger.debug("MaxForwards header not set. Set to default value 10");
            ...
            if (_logger.isDebugEnabled())
                _logger.debug("callId {} : {}", callid, callidStored);
            ...
            if (!"".equals(result)) {
                if (_logger.isDebugEnabled())
                    _logger.debug("Send message error. Missing parameters : {}", result);
            ...
            // Create the request
            Request request = messageFactory.createRequest(requestURI, command, callIdHeader, cSeqHeader, fromHeader,
            ...
            if (contact != null) {
                ...
                if (_logger.isDebugEnabled())
                    _logger.debug("contact header not set.");
            }
            ...
            if (body != null && !"".equals(body)) {
                ...
                request.setContent(body.getBytes(), contentTypeHeader);// request.setContent(contents,
                                                                       // contentTypeHeader);
            }
            ...
            if (event != null) {
                if (_logger.isDebugEnabled())
                    _logger.debug("Event header set to {}", event);
                EventHeader eventHeader = headerFactory.createEventHeader(event);
                request.addHeader(eventHeader);
            } else {
                if (_logger.isDebugEnabled())
                    _logger.debug("Event header not set.");
            }
            ...
            if (inviteTid.getDialog() == null) {
                if (_logger.isDebugEnabled())
                    _logger.debug("inviteTid.getDialog() is NULL");
            }
            ...
            if (_logger.isDebugEnabled())
                _logger.debug("sends {}", command);
            inviteTid.sendRequest();

            if ("INVITE".equals(command)) {
                // if ( _logger.isWarnEnabled()) _logger.warn("SND INVITE : " +
                // request.getHeader("Call-ID").toString().substring("Call-ID:
                // ".length()).replaceAll("\\r|\\n", "")+ ", From = " +
                // request.getHeader("From").toString().replaceAll("\\r|\\n", "")+ ", From = " +
                // request.getHeader("To").toString().replaceAll("\\r|\\n", "") + "," +
                // LocalDateTime.now().format(formatter));
            }

            if (theDial == null) {
                if (_logger.isDebugEnabled())
                    _logger.debug("theDialog is NULL");
            }
            ...
            if (_logger.isDebugEnabled())
                _logger.debug("setTheCallId with {}", callid);
    ...
    private void sendPrackRequest(SampleResult res, String command) {
        if (_logger.isDebugEnabled())
            _logger.debug("sendPrackRequestMessage");
        try {

            if (_logger.isDebugEnabled()) {
                _logger.debug("sendPrackRequestMessage based on {} : {}",
                        theSessionData.getLastResponse().getStatusCode(),
            ...
            if (relatedTransactionNb != null && !"".equals(relatedTransactionNb)
                    && !"None".equals(relatedTransactionNb)) {
            ...
            if (reqUri != null) {
                if (_logger.isDebugEnabled())
                    _logger.debug("Request-URI header set : {}", reqUri);
            ...
            if (rack != null) {
                ...
                if (rack != null && racksplit != null && racksplit.length == 3) {
                    try {
                        rackHeader = headerFactory.createRAckHeader(Integer.parseInt(racksplit[0]),
                                Integer.parseInt(racksplit[1]), racksplit[2]);
                        prackRequest.removeHeader("RAck");
                        prackRequest.setHeader(rackHeader);
                        if (_logger.isDebugEnabled())
                            _logger.debug("Rack header tag set : {}", rack);
                    } catch (NumberFormatException nfe) {
                        if (_logger.isDebugEnabled())
                            _logger.debug("Rack header tag not set. Input badly formated : {} : {}", rack, nfe);
                    } catch (InvalidArgumentException iae) {
                        if (_logger.isDebugEnabled())
                            _logger.debug("Rack header tag not set. Invalid argument : {} : {}", rack, iae);
                    }
                }
            }
    ...
    private void sendAckRequest(SampleResult res) {
        if (_logger.isDebugEnabled())
            _logger.debug("sendAckRequest");

        try {
            ClientTransaction inviteTid = (ClientTransaction) theSessionData.getATransaction(transactionNb)
                    .getTheTransaction();

            // Check if last return code was an error one. (in such case, the ACK is sent
            // back automatically by the stack)
            if (theSessionData.getATransaction(transactionNb).getLastResponse().getStatusCode() >= 300) {
                if (_logger.isDebugEnabled())
                    _logger.debug("sendAckRequest: not needed as last answer was {}",
                            theSessionData.getATransaction(transactionNb).getLastResponse().getStatusCode());
                res.setResponseMessage("sendAckRequest: not needed as last answer was "
                        + theSessionData.getATransaction(transactionNb).getLastResponse().getStatusCode());
                res.setSuccessful(true);
                return;
            }
            ...
            if (inviteTid != null) {
                ...
                try {
                    ...
                    if (cseqString != null) {
                        ...
                        if (cseqparts.length == 2) {
                            ...
                            if (_logger.isDebugEnabled())
                                _logger.debug("Cannot creat ACK. Cseq header badly decoded");
                        ...
                        if (_logger.isInfoEnabled())
                            _logger.info("CSeq header not set.");
                        ...
                        if (response == null) {
                            if (_logger.isDebugEnabled())
                                _logger.debug("Cannot creat ACK. CSeq header not set. Response is NULL");
                            ...
                            if (_logger.isDebugEnabled())
                                _logger.debug(
                                        "Cannot creat ACK. CSeq header not set. Response.getheader(CSeq) is NULL");
                            res.setResponseMessage(
                                    "Cannot creat ACK. CSeq header not set. Response CSeq header is NULL");
                            res.setSuccessful(false);
                            return;
                        } else if (dialog == null) {
                            if (_logger.isDebugEnabled())
                                _logger.debug("Cannot creat ACK. CSeq header not set. Dialog is NULL and {}",
                                        inviteTid.getDialog());
                        ...
                        SIPDialog d= (SIPDialog) dialog;
                        _logger.info("SRD {}",d.getLastTransaction().getOriginalRequest().getSIPHeaderListLowerCase("Record-Route"));

                        ackRequest = dialog
                                .createAck(((CSeqHeader) response.getHeader(CSeqHeader.NAME)).getSeqNumber());
                    }

                    //// DAV: To and From headers to force if related transaction not enough !
                    //// create Client transaction before or after ////
                    // From header
                    FromHeader fromHeader = null;
                    String from = extractOneValue(headersMap.removeHeaders("From"));
                    String tag;
                ...
                if (_logger.isDebugEnabled())
                    _logger.debug("Cannot send Ack, no transaction stored");
    ...
    private void sendRequestInDialog(SampleResult res, String command) {
        if (_logger.isDebugEnabled())
            _logger.debug("sendRequestInDialog {}", command);
        String result = "";
        try {
            // Get the Transaction.
            ...
            if (relatedTransactionNb != null && !"".equals(relatedTransactionNb)
                ...
                if (_logger.isInfoEnabled())
                    _logger.info("sendRequestInDialog with  relatedTransactionNb = {}", relatedTransactionNb);
            ...
            if (relatedTransactionNb != null && !"".equals(relatedTransactionNb)
                ...
                // Create the request
                request = messageFactory.createRequest(requestURI, command, callIdHeader, cSeqHeader, fromHeader,
                        toHeader, viaHeaders, maxForwards);
                ...
                if (_logger.isDebugEnabled())
                    _logger.debug("Cseq string value = {}", cseqString);
                CSeqHeader cSeqHeader = null;
                if (cseqString != null) {
                    String[] cseqparts = GuiHeadersParser.decodeCseq(cseqString);
                    if (cseqparts.length == 2) {
                        ...
                        if (_logger.isDebugEnabled())
                            _logger.debug("Cseq header not set. Badly decoded");
                    }
                } else {
                    if (_logger.isDebugEnabled())
                        _logger.debug("Cseq header not set. No value");
                }
                if (cSeqHeader != null) {
            ...
            if (contact != null) {
                if (_logger.isDebugEnabled())
                    _logger.debug("contact header set by scenario.");
                Address address = addressFactory.createAddress(contact);
                contactHeader = headerFactory.createContactHeader(address);
                ...
                if (_logger.isDebugEnabled())
                    _logger.debug("contact header automatically set.");
            }
    ...
    private void sendCancelRequest(SampleResult res, String command) {
        if (_logger.isDebugEnabled())
            _logger.debug("sendCancelRequest");
        ...
        try {
            ...
            if (relatedTransactionNb != null && !"".equals(relatedTransactionNb) && !"None".equals(relatedTransactionNb)
                    && theSessionData.getATransaction(relatedTransactionNb) != null) {
                inviteTid = (ClientTransaction) theSessionData.getATransaction(relatedTransactionNb)
                        .getTheTransaction();
            } else {
                inviteTid = (ClientTransaction) theSessionData.getATransaction(theSessionData.getPreviousTransaction())
                        .getTheTransaction();
            }
    ...
    private void sendAnswerMessage(SampleResult res) {
        if (_logger.isDebugEnabled())
            _logger.debug("sendAnswerMessage");

        // Get the Transaction.
        SIPServerTransactionImpl aReq = (SIPServerTransactionImpl) theSessionData.getATransaction(transactionNb)
                .getTheTransaction();
        if (aReq == null) {
            if (_logger.isDebugEnabled())
                _logger.debug("sendAnswerMessage error : no stored SIPServerTransactionImpl");

            if (relatedTransactionNb != null && !"".equals(relatedTransactionNb)) {
                aReq = (SIPServerTransactionImpl) theSessionData.getATransaction(relatedTransactionNb)
                        .getTheTransaction();
                if (aReq == null) {
                    if (_logger.isDebugEnabled())
                        _logger.debug("sendAnswerMessage error : no stored SIPServerTransactionImpl");
        ...
        try {
            ...
            if (isReliable && respCode >= 180 && respCode < 200) {
                ...
                // _logger.debug("RSeq header before addHeaders :" +
                // sessionProgress.getHeader("RSeq"));
                ...
                if (rseq != null) {
                    try {
                        ...
                        if (_logger.isDebugEnabled())
                            _logger.debug("Rseq header tag set : {}", rseq);
                    } catch (NumberFormatException nfe) {
                ...
                theSessionData.getTheDialog().sendReliableProvisionalResponse(sessionProgress);// (Response)
                                                                                               // sessionProgress.clone());
                ...
                if (to != null) {
                    ...
                    if (tag != null) {
                        ...
                        if (_logger.isDebugEnabled())
                            _logger.debug("To header tag not set.");
                    }
                }
                ...
                try {
                    ...
                    if (st.getDialog() == null) {
                        if (_logger.isDebugEnabled())
                            _logger.debug("st.enableRetransmissionAlerts()");
            ...
            if (_logger.isDebugEnabled())
                _logger.debug("Send response message exception for {} : {} : {} : {}",
                        new Object[] { getName(), dialId, prevReqDialId, ex });
            res.setResponseMessage("Send response message exception for " + getName() + ":" + dialId + ":"
                    + prevReqDialId + " : " + ex.toString());
            res.setSuccessful(false);
        }
    }
    ...
    private void receiveMessage(SampleResult res) {
        ...
        if (_logger.isDebugEnabled())
            _logger.debug("samplerTimeout = {}", timeout);
    ...
    private void receiveRequestMessage(SampleResult res) {
        ...
        try {
            ...
            if (_logger.isDebugEnabled())
                _logger.debug("receiveRequestMessage idHeader name = {}",
                        this.sipNodeContext.getSipNode().getIdentificationHeader());
            // Get identification header
            String id = SipString.cleanString(extractOneValue(
                    headersMap.removeHeaders(this.sipNodeContext.getSipNode().getIdentificationHeader())));
            if (_logger.isDebugEnabled())
                _logger.debug("receiveRequestMessage idHeader name = {} , and value ={}",
                        this.sipNodeContext.getSipNode().getIdentificationHeader(), id);
            id = SipString.extractAddress(id);

            // Get request queue
            ArrayBlockingQueue<SipRequestTransaction> reqQueue = sipNodeContext
                    .checkRequestQueue(id + "_" + getProperty("sip.message.command").getStringValue());

            if (reqQueue == null) {
                if (_logger.isDebugEnabled())
                    _logger.debug("req queue not created for id {}", id);
                ...
                if (_logger.isErrorEnabled()) {
                    long diff = dateToString() - start;
                    if (diff > 5 && _logger.isWarnEnabled())
                        _logger.warn("reqQueue.poll for id {}_{} -> POLL took {}", id,
                                getProperty("sip.message.command").getStringValue(), diff);
                }

                if (aReq == null) {
                    // No message received
                    if (_logger.isDebugEnabled())
                        _logger.debug("incoming request for id {} not received within timeout {}", id, timeout);
                    if (isOptional) {
                        res.setResponseMessage(
                                "Optional: no message received for id " + id + " within timeout " + timeout);
                        res.setSuccessful(true);
                    } else {
                        res.setResponseMessage(
                                "failed: no message received for id " + id + " within timeout " + timeout);
                        res.setSuccessful(false);
                    }
                    return;
                } else {
                    ...
                    if (_logger.isDebugEnabled())
                        _logger.debug("incoming request callid {}.", callid);

                    if ("INVITE".equals(rcvReq.getMethod())) {
                        // if ( _logger.isWarnEnabled()) _logger.warn("RCV INVITE : " +
                        // rcvReq.getHeader("Call-ID").toString().substring("Call-ID:
                        // ".length()).replaceAll("\\r|\\n", "")+ ", From = " +
                        // rcvReq.getHeader("From").toString().replaceAll("\\r|\\n", "")+ ", From = " +
                        // rcvReq.getHeader("To").toString().replaceAll("\\r|\\n", "") + "," +
                        // LocalDateTime.now().format(formatter));
                    }
                    ...
                    Dialog theDial = null;
                    if (_logger.isDebugEnabled())
                        _logger.debug("is Dialog exist for method {}, callid ={}", rcvReq.getMethod(), callid);
                    if (aReq.getTheTransaction() != null) {
                        if (aReq.getTheTransaction().getDialog() == null
                                && DialogCreatingRequests.isCreating(rcvReq.getMethod())) {
                            if (_logger.isDebugEnabled())
                                _logger.debug("Dialog does not exist, try to create it");
                            try {
                                theDial = sipProvider.getNewDialog(aReq.getTheTransaction());
                            } catch (SipException se) {
                                if (_logger.isDebugEnabled())
                                    _logger.debug("Dialog does not exist, cannot create it : {}", se);
                            }
                        } else {
            ...
            if (_logger.isDebugEnabled())
                _logger.debug("receiveRequestMessage exception : {}", e);
    ...
    private void receiveAnswerMessage(SampleResult res) {
        ...
        try {
            ...
            // Get identification header
            String id = SipString.cleanString(
                    extractOneValue(headersMap.removeHeaders(sipNodeContext.getSipNode().getIdentificationHeader())));
            String callidStored = theSessionData.getTheCallId();
            if (callidStored != null && !"".equals(callidStored)) {
                id = callidStored;
            }
            if (_logger.isDebugEnabled())
                _logger.debug("resp queue for id {}.", id);

            // Forking case: if relatedTransactionNb != null
            if (relatedTransactionNb != null && !"".equals(relatedTransactionNb)
                    && !"None".equals(relatedTransactionNb)) {
                if (_logger.isDebugEnabled())
                    _logger.debug("receiveAnswerMessage with relatedTransactionNb =  {}.", relatedTransactionNb);
                try {
                    theSessionData.setADialog(relatedTransactionNb, sipProvider
                            .getNewDialog(theSessionData.getATransaction(relatedTransactionNb).getTheTransaction()));
                } catch (Throwable t) {
                    if (_logger.isDebugEnabled())
                        _logger.debug("receiveAnswerMessage with relatedTransactionNb = {}  throwable {}",
                                relatedTransactionNb, t.getMessage());
                }
            }
            ...
            // Get response queue
            ArrayBlockingQueue<SipResponseTransaction> respQueue = sipNodeContext.checkResponseQueue(
                    id + "_" + getProperty("sip.message.response.code").getStringValue() + "_" + method);

            if (respQueue == null) {
                if (_logger.isDebugEnabled())
                    _logger.debug("resp queue not created for id {}.", id);
                ...
                while (retry) {
                    ...
                    if (aResp == null) {
                        // No message received
                        if (_logger.isDebugEnabled())
                            _logger.debug("incoming response for id {} not received within timeout {}", id, timeout);
                        if (isOptional) {
                            res.setResponseMessage("Optional: no response message received for id " + id
                                    + " within timeout " + timeout);
                            res.setSuccessful(true);
                        } else {
                            res.setResponseMessage(
                                    "failed: no response message received for id " + id + " within timeout " + timeout);
                            res.setSuccessful(false);
                        }
                    } else {
                        // Message received
                        if (_logger.isDebugEnabled())
                            _logger.debug(
                                    "receiveAnswerMessage: poll in responsesQeueues for id = {}. remaining queue size = {}",
                                    id, respQueue.size());
                        ...
                        if (aResp.getTheResponse() == null) {
                            if (_logger.isDebugEnabled())
                                _logger.debug("No Response got for SipResponseTransaction");
                        }

                        // Get Call-ID
                        CallIdHeader cidh = (CallIdHeader) rcvResp.getHeader("Call-ID");
                        String callid = SipString
                                .cleanString(rcvResp.getHeader("Call-ID").toString().substring("Call-ID".length() + 2));
                        ...
                        if (ignoreRetransmission && isAlreadyReceived(rcvResp, theTransData.getLastResponse())) {
                            ...
                            // if (theTransData.getLastResponse() == null ||
                            // theTransData.getLastResponse().getStatusCode() != rcvResp.getStatusCode()) {
                            ...
                            if (aResp.getTheTransaction() == null) {
                                if (_logger.isDebugEnabled())
                                    _logger.debug("No transaction got for SipResponseTransaction");
                            } else {
                            ...
                            if (!Request.PRACK.equals(cseqH.getMethod())) { // aResp.getTheTransaction().getMethod())) {
                                if (_logger.isDebugEnabled())
                                    _logger.debug("setLastResponse for method {}", cseqH.getMethod()); // aResp.getTheTransaction().getMethod());
    ...
    private boolean isAlreadyReceived(Response rcvResp, Response lastStoredResp) {
        if (lastStoredResp != null) {
            if (lastStoredResp.toString().equals(rcvResp.toString())) {
                if (_logger.isDebugEnabled())
                    _logger.debug("response already received: should be retransmission {} : {}",
                            lastStoredResp.toString(), rcvResp.toString());
                return true;
            } else {
                if (_logger.isDebugEnabled())
                    _logger.debug("response not already received: it seems no to be a retransmission {} : {}",
                            lastStoredResp.toString(), rcvResp.toString());
                return false;
            }
        } else {
            if (_logger.isDebugEnabled())
                _logger.debug("No lastStored response. response received: {}", rcvResp.toString());
        }
        return false;
    }
    ...
    private void checkReceivedRequest(Request aReq, SampleResult res) {
        if (_logger.isDebugEnabled())
            _logger.debug("checkReceivedRequest ");
        String diff = "";
        ...
        if (!aReq.getMethod().equals(expectedCmd)) {
            ...
            if (_logger.isDebugEnabled())
                _logger.debug("checkReceivedMessage  Method check failed: {} : {}.", expectedCmd, aReq.getMethod());
            res.setSuccessful(false);
        }
    ...
    private void checkReceivedResponse(Response rcvResp, SampleResult res) {
        if (_logger.isDebugEnabled())
            _logger.debug("checkReceivedResponse");
        String diff = "";
        ...
        if (rcvResp.getStatusCode() != Integer.parseInt(expectedRespCode)) {
            ...
            if (_logger.isDebugEnabled())
                _logger.debug("checkReceivedResponse Response code check failed: {} : {}.", expectedRespCode,
                        rcvResp.getStatusCode());
        }
    ...
    private String checkHeaders(Message rcvMsg, String diff) {
        ...
        for (java.util.Map.Entry<String, List<String>> entry : headersMap.entrySet()) {
            if (_logger.isDebugEnabled())
                _logger.debug("checkReceivedMessage for key {} : {}", entry.getKey(), entry.getValue());
                ...
                if ("Request-URI".equals(entry.getKey())) {
                    if (rcvMsg instanceof Request) {
                        if (entry.getValue().size() != 1) {
                            ...
                            diff = diff + entry.getKey() + " check failed: Request-URI has 1 value, not "
                                    + entry.getValue().size() + ".\n";
                            if (_logger.isDebugEnabled())
                                _logger.debug("checkReceivedMessage {} check failed: Request-URI has 1 value, not {}.",
                            ...
                            if (compVal.startsWith("STORE:")) {
                                ...
                                String eV = entry.getValue().get(0); // SipString.cleanString(entry.getKey() +":" +
                                                                     // eVL.get(i));
                                ...
                                if (!checkOneHeaderWithPattern(aH, eV)) { // eV.equals(aH)) {
                                    ...
                                    diff = diff + entry.getKey() + " check with pattern failed: " + eV + ":" + aH
                                            + ".\n";
                                    if (_logger.isDebugEnabled())
                                        _logger.debug("checkReceivedMessage {} check with pattern failed: {} : {}.",
                                                entry.getKey(), eV, aH);
                                }
                            } else {
                                if (!checkOneHeader(reqURIrcv, compVal)) { // eV.equals(aH)) {
                                    ...
                                    diff = diff + entry.getKey() + " check failed: " + reqURIrcv + ":" + compVal
                                            + ".\n";
                                    if (_logger.isDebugEnabled())
                                        _logger.debug("checkReceivedMessage {} check failed: {} : {}.", entry.getKey(),
                                                reqURIrcv, compVal);
                                }
                            }
                        }
                        ...
                        diff = diff + entry.getKey()
                                + " check failed: received message not a request. It has no Request-URI.\n";
                        if (_logger.isDebugEnabled())
                            _logger.debug(
                                    "checkReceivedMessage {} check failed: not present in message. It has no Request-URI.",
                                    entry.getKey());
                    }
                } else {
                    ...
                    if (!rcvMsg.getHeaders(entry.getKey()).hasNext()) {
                        ...
                        if (!(eVL.size() == 1 && eVL.get(0) != null && eVL.get(0).startsWith("NODEF:"))) {
                            ...
                            if (_logger.isDebugEnabled())
                                _logger.debug("checkReceivedMessage {} check failed: not present in message.",
                                        entry.getKey());
                        }
                    } else {
                        for (Iterator<Header> iterator = rcvMsg.getHeaders(entry.getKey()); iterator.hasNext();) { // List
                                                                                                                   // of
                                                                                                                   // values
                                                                                                                   // from
                                                                                                                   // the
                            ...
                            if (i >= eVL.size()) {
                                ...
                                if (eVLi.startsWith("STORE:")) {
                                    ...
                                    if (!checkOneHeaderWithPattern(aH, eV)) { // eV.equals(aH)) {
                                        ...
                                        if (_logger.isDebugEnabled())
                                            _logger.debug(
                                                    "checkReceivedMessage {}  check with pattern failed: {} : {}.",
                                                    entry.getKey(), eV, aH);
                                    }
                                }
                                    ...
                                    if (!eVLi.startsWith("NODEF:")) {
                                        ...
                                        if (!checkOneHeader(aH, eV)) { // eV.equals(aH)) {
                                            ...
                                            if (_logger.isDebugEnabled())
                                                _logger.debug("checkReceivedMessage {} check failed: {} : {}.",
                                                        entry.getKey(), eV, aH);
                                        }
                                    }
                                }
                        ...
                        if (i < eVL.size()) {
                            ...
                            diff = diff + entry.getKey() + " check failed: more values to check remain : "
                                    + (eVL.size() - i) + ".\n";
                        }
                    }
                }
        ...
        if (bodyValueToCheck != null && !"".equals(bodyValueToCheck)) {
            ...
            if (rcvMsg != null && rcvMsg.getContent() != null) {
                try {
                    ...
                    if (_logger.isDebugEnabled())
                        _logger.debug("checkHeaders: body decoding exception : {}", e);
                }
            }
            // replaceAll("[\n\r]", "");
            ...
            if (bodyValueToCheck.startsWith("STORE:")) {
                ...
                if (!checkOneHeaderWithPattern(bodyToValidate, bodyValueToCheck)) { // eV.equals(aH)) {
                    ...
                    if (_logger.isDebugEnabled())
                        _logger.debug("checkReceivedMessage body check with pattern failed: {} : {}.", bodyValueToCheck,
                                bodyToValidate);
                }
            } else {
    ...
    private boolean checkOneHeader(String receivedValue, String comparisonValue) {
        ...
        if (_logger.isDebugEnabled())
            _logger.debug("checkOneHeader {} : {} : {} : {}", new String[] { tagrcv, tagcomp, valrcv, valcomp });
    ...
    private boolean checkOneHeaderWithPattern(String receivedValue, String pattern) {
         ...
         */
        if (_logger.isDebugEnabled())
            _logger.debug("checkOneHeaderWithPattern {} with pattern : {}", receivedValue, pattern);
        ...
        for (String oneViaString : viaString) {
            if (oneViaString != null && !"".equals(oneViaString)) {
                ...
                if (via != null) {
                    ...
                    if (_logger.isDebugEnabled())
                        _logger.debug("createViaHeader add {} : {} : {} : {}",
                                new Object[] { ipAddress, ipPort, transport, branch });
    ...
    private ArrayList<RouteHeader> createRouteHeader() throws ParseException, InvalidArgumentException {
        ...
        for (String oneRouteString : routeString) {
            if (oneRouteString != null && !"".equals(oneRouteString)) {
                ...
                if (_logger.isDebugEnabled())
                    _logger.debug("createRouteHeader add {}", oneRouteString);
    ...
    public ToHeader createToHeader(String toString) {
        ...
        if (toString != null) {
            ...
            try {
                ...
                if (_logger.isDebugEnabled())
                    _logger.debug("createToHeader extTo = {}", extTo);
                ...
                if (params != null && params.length != 0) {
                    for (String oneParam : params) {
                        ...
                        if (_logger.isDebugEnabled())
                            _logger.debug("createToHeader oneParam = {} : {}", var, val);
                ...
                if (_logger.isDebugEnabled())
                    _logger.debug("ParseException when creating To header : {}", e);
            }
        } else {
            if (_logger.isDebugEnabled())
                _logger.debug("To is null...");
        }
        return toHeader;
    ...
    public FromHeader createFromHeader(String fromString) {
        ...
        if (fromString != null) {
            ...
            try {
                ...
                if (_logger.isDebugEnabled())
                    _logger.debug("ParseException when creating From header : {}", e);
            }
        } else {
            if (_logger.isDebugEnabled())
                _logger.debug("From is null...");
        }
        return fromHeader;
    ...
    public URI createReqURITelOrSip(String reqUri) {
        ...
        if (reqUri != null) {
            if (reqUri.startsWith("sip:") || reqUri.startsWith("sips:")) {
                ...
                if (_logger.isDebugEnabled())
                    _logger.debug("Request-URI type not supported.");
            }
        } else {
            if (_logger.isDebugEnabled())
                _logger.debug("Request-URI header not set.");
    ...
    private void startAndSetSplitInClientTransaction(String command, ClientTransaction theTrans) {
        // create a Simon split for responsetime statisticts. Put it in the session
        // application data
        //// Split split =
        // SimonManager.getStopwatch(SipCounters.SIP_NODE_RESPONSE_TIME+"."+command+"."+sipNodeContext.getSipNode().getSipNodeName().replace(".",
        // "_")).start();
        // theTrans.setApplicationData(sipNodeContext.getSplitAndStart(SipCounters.SIP_NODE_RESPONSE_TIME+"."+command+"."+sipNodeContext.getSipNode().getSipNodeName().replace(".",
        // "_"))); //theSessionData.setResponseTimeSplit(split);
        /*
         * JMeterVariables variables = getThreadContext().getVariables();
         * StatisticsManager statManager = (StatisticsManager)
         * variables.getObject("StatisticsManager"); if (statManager != null) {
         * statManager.createAndStartSplit(SipCounters.SIP_NODE_RESPONSE_TIME+"."+
         * command+"."+sipNodeContext.getSipNode().getSipNodeName().replace(".", "_"),
         * theTrans.getDialog().getDialogId()); } else {
